...
class RAGSplitter:
    ...
    def __init__(
            self,
            split_by: Literal["word", "sentence", "page", "passage", "token", "custom"] = "word",
        ...
        if split_by not in ["word", "sentence", "page", "passage", "token", "custom"]:
            raise ValueError("split_by must be one of 'word', 'sentence', 'page','passage' or 'token'.")
        if split_length <= 0:
    ...
    def run(self, documents: List[Document]):
        ...
        for doc in documents:
            ...
            if self.file_name is not None and self.file_name != '':
                metadata["file_name"] = self.file_name  # 将文档的名称存为meta data
            split_docs += self._create_docs_from_splits(
                text_splits=text_splits, splits_pages=splits_pages, splits_start_idxs=splits_start_idxs, meta=metadata
            )
        return {"documents": split_docs}

    def split_string(self, s, length=20):
        return [s[i:i + length] for i in range(0, len(s), length)]

    def _split_into_units(self, text: str, split_by: Literal["word", "sentence", "passage", "page", "token", "custom"],
                          max_length=1024) -> List[str]:
        if split_by == "page":
            self.split_at = "\f"
        ...
        try:
            if split_by == "sentence":
                result = re.split(self.split_at, text)
                for (i, r) in enumerate(result):
                    if r == '':
                        continue
                    index = text.index(r)
                    if index + len(r) >= len(text):
                        units.append(r)
                        break
                    limiter = text[index + len(r)]
                    unit = r + limiter

                    if len(unit) > max_length:
                        splited = self.split_string(s=unit, length=max_length)
                        units.extend(splited)
                        continue

                    units.append(unit)
            elif split_by == "custom":
                units = re.split(self.split_at, text)
            else:
