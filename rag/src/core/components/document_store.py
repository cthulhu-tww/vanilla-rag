...
class MilvusDocumentStore:
    ...
    def __init__(
        ...
        self.enable_text_match = enable_text_match
        ...
        if metas:
            ...
            for key, value in metas[0].items():
                ...
                # Datatype is a string/varchar equivalent
                    ...
                    if self.enable_text_match:
                        fields.append(FieldSchema(key, DataType.VARCHAR, max_length=65_535, enable_analyzer=True,
                                                  enable_match=True))
                    else:
    ...
    def _hybrid_retrieval(
        ...
        # 将res中每个检索到的chunk的value赋值给doc中的value
        hits = res[0]
        for hit, doc in zip(hits, docs):
            doc.score = hit.score
            doc.meta['collection_name'] = self.collection_name
        return docs
